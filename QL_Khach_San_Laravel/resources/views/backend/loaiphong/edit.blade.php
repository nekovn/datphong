...
<form method="post" action="{{ route('loaiphong.update', ['loaiphong' => $loaiphong->lp_ma]) }}" enctype="multipart/form-data">
    <input type="hidden" name="_method" value="PUT" />
    {{ csrf_field() }}
  
  <div class="form-group">
    <label for="lp_ten">Tên Loại Phòng</label>
    <input type="text" class="form-control" id="lp_ten" name="lp_ten" value="{{ old('lp_ten', $loaiphong->lp_ten) }}"">
  </div>
  <div class="form-group">
    <label for="lp_giaGoc">Giá bán</label>
    <input type="text" class="form-control" id="lp_giaBan" name="lp_giaBan" value="{{ old('lp_giaBan', $loaiphong->lp_giaBan) }}">
  </div>
  <div class="form-group">
  	 <label>Hình đại diện</label><br>
  	<img src="{{ asset('/storage/photos/' . $loaiphong->lp_hinh) }}" class="img-list" height="200" width="autoUnmask" />
    <div class="file-loading">
     
      <input id="lp_hinh" type="file" height="300" width="300"name="lp_hinh">
    </div>
  </div>
  <div class="form-group">
    <label for="lp_thongTin">Thông tin</label>
    <input type="text" class="form-control" id="lp_thongTin" name="lp_thongTin" value="{{ old('lp_thongTin',$loaiphong->lp_thongTin) }}">
  </div>
